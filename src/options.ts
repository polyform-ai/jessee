...
async function render(message = ""): Promise<void> {
  ...
  document.querySelector("#saveTemplate")?.addEventListener("click", async () => {
    ...
    const name = document.querySelector<HTMLInputElement>("#editTemplateName")?.value.trim() ?? "";
    const instructions = document.querySelector<HTMLTextAreaElement>("#editTemplateInstructions")?.value.trim() ?? "";
