...
Version 0.2.1 is the first supported native Mac release. Credentials from the retired browser previews were never stored in the Mac Keychain. Local pre-release Mac builds used development-only signing. Current signed releases offer Polyform Covered email authentication or an OpenAI key stored in the login Keychain.
