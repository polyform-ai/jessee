...
Version 0.2.1 is the first supported native Mac release. Credentials from the retired browser previews were never stored in the Mac Keychain. Local pre-release Mac builds used development-only signing. The current signed app asks for an OpenAI key once and keeps it in the login Keychain; Polyform Covered remains feature-gated until its managed service is release-ready.
