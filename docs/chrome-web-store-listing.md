...
4. Edit the AI-generated story directly and click any selected screenshot to step through alternatives.
5. Generate an evidence-backed PDF that is easy to read, share, and feed into another AI workflow.
6. Return to the local walkthrough library to replay the recording, revise its story, or download a fresh PDF.
...
Recordings, screenshots, and generated PDFs are stored locally on your computer. When you choose Create Plan, JesSee sends the capture evidence and narration needed to create that document to OpenAI using the API key you provide. Generating or downloading the reviewed PDF makes no additional AI request. The open-source build has no analytics or telemetry endpoint configured.
